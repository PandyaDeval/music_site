...
<script type="text/javascript">
   ...
   function search_submit(a){
	  var search_term=document.getElementById('myInput').value;
	  location='test3.php?search_term='+search_term;
	
   }
   
   function view_search_results(a){
...
<div id="leftbar">
   ...
   <center><br><br>
   <form autocomplete="off" method="POST" id="create_playlist_form">
...
<div style="color:black;position:absolute;top:20px;left:60%">
         <div class="autocomplete" style="width:250px;">
         <input id="myInput" type="text" name="myCountry">
         </div>
         <button onclick="search_submit(<?php echo $no_of_playlists;?>)">Submit</button>
</div>
...
<?php
if($search_term!=''){
echo "<script>view_search_results($no_of_playlists);</script>";
$search_qry="SELECT * FROM `songs` where tags LIKE '%$search_term%'";
$search_data=mysqli_query($con,$search_qry);
echo "<table>";
while($search_row=mysqli_fetch_array($search_data)){
	$album_art=$search_row[4];
	$title=$search_row[1];
	$artist=$search_row[3];
	$link=$search_row[2];
	$id=$search_row[0];
